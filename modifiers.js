...
function renderElExtra(el, d) {
  ...
  // ── GlassButton ─────────────────────────────────────────────
  if (el.type === 'GlassButton') {
    var rimOp  = el.glassRimOp  !== undefined ? el.glassRimOp  : 0.7;
    var tintOp = el.glassTintOp !== undefined ? el.glassTintOp : 0.1;

    _liquidEnsureFilter(el.id, el.glassNoise || 0.008, el.glassDistort || 60);
    ...
    // ::after giả — backdrop-filter + SVG distortion
    var glassLayer = document.createElement('div');
    glassLayer.style.cssText = [
      'position:absolute', 'inset:0', 'z-index:-1',
      'border-radius:9999px',
      'backdrop-filter:blur(' + (el.glassBlur || 0) + 'px)',
      '-webkit-backdrop-filter:blur(' + (el.glassBlur || 0) + 'px)',
      'filter:url(#lf-filter-' + el.id + ')',
      '-webkit-filter:url(#lf-filter-' + el.id + ')',
      'overflow:hidden',
      'isolation:isolate',
    ...
    // ::before giả — rim light + tint (giống style.css .glassBtn::before)
    var rimLayer = document.createElement('div');
    rimLayer.style.cssText = [
      'position:absolute', 'inset:0', 'z-index:0',
      'border-radius:9999px',
      'background:rgba(255,255,255,' + tintOp + ')',
      'box-shadow:inset 2px 2px 0px -2px rgba(255,255,255,' + rimOp + '),'
               + 'inset 0 0 3px 1px rgba(255,255,255,' + (rimOp * 0.6).toFixed(2) + ')',
      'pointer-events:none',
    ].join(';');
...
function renderPropsExtra(el, id) {
  ...
  // ── GlassButton ──────────────────────────────────────────────
  if (el.type === 'GlassButton') {
    h += modSec('🫧 Glass Button',
      modTr(id, 'Icon',       'icon',         el.icon         || '☀') +
      modNr(id, 'Blur (px)',  'glassBlur',    0, 20,  el.glassBlur    !== undefined ? el.glassBlur    : 0,   0.5) +
      modNr(id, 'Distort',    'glassDistort', 0, 120, el.glassDistort !== undefined ? el.glassDistort : 60,  1)   +
      modNr(id, 'Tint Op',    'glassTintOp',  0, 1,   el.glassTintOp  !== undefined ? el.glassTintOp  : 0.1, 0.01)+
      modNr(id, 'Rim Op',     'glassRimOp',   0, 1,   el.glassRimOp   !== undefined ? el.glassRimOp   : 0.7, 0.01)+
      '<div class="pr" style="font-size:9px;color:var(--cy)">💡 Đặt GlassButton lên nền có texture để thấy hiệu ứng</div>'
    );
  }
...
// LiquidFrame — feTurbulence (Version 1)
...
// GlassButton — feImage noise PNG (Version 2)
function _glassEnsureFilter(elId, distort) {
  distort = distort !== undefined ? distort : 60;
