...
function mkEl(type, x, y, w, h) {
  ...
  var o = {
    id: 'el_' + (++idc), type: type,
    x: x, y: y, w: w, h: h,
    name: type + idc, mods: {}, rot: 0, parentId: null, warp: null,
    // UDim2 Position (root: scale=0, offset=px)
    psX: 0, poX: x,
    psY: 0, poY: y,
    // UDim2 Size (root: scale=0, offset=px)
    ssW: 0, soW: w,
    ssH: 0, soH: h
  };
...
function startGroupDrag(e) {
  saveH();
  var grp = selGroup.map(getEl).filter(Boolean);
  // snapshot world coords + UDim2 ban đầu
  var snaps = grp.map(function(el) {
    syncWorldRect(el);
    return { el: el, x: el.x, y: el.y };
  });
  var smx = e.clientX, smy = e.clientY;
 
  function mm(ev) {
    var dx = ev.clientX - smx, dy = ev.clientY - smy;
    var tempSnaps = snaps.map(function(s) {
      return { x: s.x + dx, y: s.y + dy, w: s.el.w, h: s.el.h };
    });
    ...
    snaps.forEach(function(s) {
      ...
      s.el.x = nx; s.el.y = ny;
      // FIX: sync UDim2 sau khi dịch chuyển
      setUDim2FromWorld(s.el, nx, ny, s.el.w, s.el.h);
      syncWorldRect(s.el);
      renderEl(s.el);
      // kéo descendants theo
      getDescendants(s.el.id).forEach(function(c) {
        syncWorldRect(c);
        renderEl(c);
      });
    });
...
function startGroupScale(pos, e) {
  ...
  // snapshot tỉ lệ của mỗi element trong group bounds
  var snaps = grp.map(function(el) {
    syncWorldRect(el);
    var a = getAbsPos(el);
    return {
  ...
  function mm(ev) {
    ...
    snaps.forEach(function(s) {
      ...
      // FIX: dùng setUDim2FromWorld thay vì gán el.x trực tiếp
      setUDim2FromWorld(el, nax, nay, nw, nh);
      syncWorldRect(el);
      renderEl(el);
      // scale descendants theo
      getDescendants(el.id).forEach(function(c) {
        syncWorldRect(c);
        renderEl(c);
      });
    });
...
function dupSel() {
  ...
  if (selGroup.length > 1) {
    ...
    newEls.forEach(function(c) {
      // giữ parentId trong group nếu parent cũng được dup
      if (c.parentId && idMap[c.parentId]) c.parentId = idMap[c.parentId];
      // dịch 20px — cập nhật cả world lẫn UDim2
      c.x += 20; c.y += 20;
      c.poX = (c.poX !== undefined && c.poX !== null) ? c.poX + 20 : c.x;
      c.poY = (c.poY !== undefined && c.poY !== null) ? c.poY + 20 : c.y;
    });
...
document.addEventListener('keydown', function(e) {
  ...
  if (e.ctrlKey && e.key === 'p') {
    ...
    var cx = child.x + child.w / 2, cy = child.y + child.h / 2;
  ...
  // Debounce saveH khi giữ arrow key
  ...
  // FIX: sau mỗi arrow key → setUDim2FromWorld + syncDescendants
  if (e.key === 'ArrowUp') {
    el.y = Math.max(0, el.y - st);
    setUDim2FromWorld(el, el.x, el.y, el.w, el.h);
    syncWorldRect(el);
    syncDescendants(el.id);
    renderEl(el);
...
function alignLeft() {
  ...
  group.forEach(function(e) {
    e.x = minX;
    setUDim2FromWorld(e, e.x, e.y, e.w, e.h);
    syncWorldRect(e);
    renderEl(e);
  });
  toast('⬤ Align Left');
...
function distributeH() {
  ...
  var minX = group[0].x;
  var maxX = group[group.length - 1].x + group[group.length - 1].w;
...
function distributeV() {
  ...
  var minY = group[0].y;
  var maxY = group[group.length - 1].y + group[group.length - 1].h;
